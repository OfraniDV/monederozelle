...
- El bloque 🏦 <b>Liquidez rápida disponible</b> muestra el equivalente aproximado en USD por banco usando la tasa de compra almacenada en la base de datos y sigue la convención `valor CUP (≈ USD)` cuando la tasa está disponible.
- Inserta un bloque 📉 <b>Detalle de deudas por agente/subcuenta</b> inmediatamente después del estado actual, listando cada saldo negativo en CUP por agente y tarjeta (enmascarada) con totales por agente y un total general; cuando existe tasa de compra, incluye la conversión aproximada a USD.
...
- Evalúa los límites mensuales sumando las SALIDAS (`importe < 0`) del mes por tarjeta CUP, muestra el tope activo en la línea ℹ️ y renderiza una tabla monoespaciada dentro de `<pre>` con columnas Banco, Tarjeta (`#1234`), `SAL`, `SALDO`, `LIBRE`, `CAP`, Estado (🟥 BLOCKED, 🟡 EXTENDABLE, 🟢 OK) y, cuando hay tasa de compra, la columna adicional `≈USD(LIBRE)` más una línea de totales aproximados en USD. El `CAP` aplica `max(0, (límite − salidas) − saldo_actual_positivo)` y MITRANSFER/BOLSA nunca aparece en esa tabla.
